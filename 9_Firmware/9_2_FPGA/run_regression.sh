...
# ===========================================================================
# PHASE 5: REAL-DATA CO-SIMULATION (ADI CN0566 golden reference)
#
# These testbenches feed actual ADI CN0566 Phaser radar data (10.525 GHz
# X-band FMCW) through the RTL and compare output bit-for-bit against
# Python golden references generated by golden_reference.py.
#
# Hex stimulus and reference files are pre-generated and committed in
# tb/cosim/real_data/hex/.  Pass criteria: exact match (MAX_ERROR=0).
# ===========================================================================
echo "--- PHASE 5: Real-Data Co-Simulation ---"

if [[ "$QUICK" -eq 0 ]]; then
    run_test "Range FFT (ADI CN0566 real data)" \
        tb/tb_range_fft_realdata_reg.vvp \
        tb/tb_range_fft_realdata.v fft_engine.v

    run_test "Doppler (ADI CN0566 real data)" \
        tb/tb_doppler_realdata_reg.vvp \
        tb/tb_doppler_realdata.v doppler_processor.v xfft_32.v fft_engine.v

    run_test "Full-chain Decim+Doppler (ADI CN0566 real data)" \
        tb/tb_fullchain_realdata_reg.vvp \
        tb/tb_fullchain_realdata.v \
        range_bin_decimator.v doppler_processor.v xfft_32.v fft_engine.v

    run_test "Full-chain MTI+CFAR (ADI CN0566 real data)" \
        tb/tb_fullchain_mti_cfar_realdata_reg.vvp \
    ...
    echo "  (skipped real-data co-sim — use without --quick)"
    SKIP=$((SKIP + 4))
fi
